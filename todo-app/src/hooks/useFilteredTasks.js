...
export function useTaskStats(tasks) {
  return useMemo(() => {
    ...
    const overdue = tasks.filter(
      (t) => t.dueDate && isPast(parseISO(t.dueDate)) && !t.completed
    ).length
    const today = tasks.filter(
      (t) => t.dueDate && isToday(parseISO(t.dueDate)) && !t.completed
    ).length
    const week = tasks.filter(
      (t) => t.dueDate && isThisWeek(parseISO(t.dueDate)) && !t.completed
    ).length
